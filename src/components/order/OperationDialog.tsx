...
export default function OperationDialog({ open, onOpenChange, order, mode, processing = false, onConfirm }: OperationDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-dialog-safe overflow-y-auto sm:max-w-3xl bg-background shadow-xl rounded-[24px]">
        <DialogHeader>
                  ...
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <div className="w-24">
                      ...
                      <NumericInput
                        ...
                        onValueChange={(value) => setQtyByItem((prev) => ({ ...prev, [item.id]: value }))}
                        className="h-8"
                      />
                    </div>
                  </div>
