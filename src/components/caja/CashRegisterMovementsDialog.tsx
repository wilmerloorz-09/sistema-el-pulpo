...
import { AlertCircle, ArrowRightLeft, Clock, Coins, DollarSign, Loader2, Minus, Plus } from "lucide-react";
...
function DenominationEditorSection({
  ...
  return (
    <div className={cn("rounded-xl border p-2.5 shadow-sm sm:rounded-2xl sm:p-3", accentClasses.border, accentClasses.background)}>
      <div className={cn(description ? "mb-2.5" : "mb-1.5")}>
        <p className={cn("text-lg font-bold", accentClasses.title)}>{title}</p>
        {description && <p className="text-xs text-muted-foreground">{description}</p>}
      </div>

      <div className="space-y-1.5 sm:space-y-2">
        {options.map((option) => {
          const isLocked = (otherCounts[option.id] ?? 0) > 0;
          return (
            <div
              key={option.id}
              className={cn(
                "grid min-h-[74px] grid-cols-[auto_minmax(0,1fr)_auto_80px] items-center gap-2 rounded-lg border bg-white px-2 py-1.5 transition-opacity sm:grid-cols-[auto_minmax(0,1fr)_auto_100px] sm:gap-4 sm:rounded-xl sm:px-3 sm:py-2",
                isLocked ? "opacity-40 border-slate-200" : "border-orange-200"
              )}
            >
            <DenominationVisual
              label={option.label}
            ...
            <div className="min-w-0 flex-1 font-sans">
              <p className="truncate text-base font-bold text-slate-950 tracking-tight">{option.label}</p>
              {showAvailable && (
                <p className="text-[13px] font-bold text-red-800 mt-0.5">
                  Disponible: {option.currentQty}
                </p>
              )}
              {!showAvailable && (
                <p className="text-[13px] font-bold text-transparent mt-0.5">Disponible: 0</p>
              )}
            </div>
            ...
            <div className="flex items-center gap-1.5">
              <Button
                ...
                onClick={() => onChange(option.id, (counts[option.id] ?? 0) - 1)}
                disabled={isLocked || (counts[option.id] ?? 0) <= 0}
              >
              ...
              <NumericInput
                ...
                disabled={isLocked}
                className="h-8 w-12 rounded-lg px-1 text-center text-xs font-semibold tabular-nums [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none sm:h-9 sm:w-14 sm:text-sm"
              />
              ...
              <Button
                ...
                className="h-8 w-8 shrink-0 rounded-lg sm:h-9 sm:w-9"
                onClick={() => onChange(option.id, (counts[option.id] ?? 0) + 1)}
                disabled={isLocked || (enforceAvailable && (counts[option.id] ?? 0) >= option.currentQty)}
              >
                <Plus className="h-3.5 w-3.5" />
              </Button>
            </div>
              <span className="text-right text-base font-bold text-slate-950 tabular-nums">
          ...
          >
            <div className={`flex flex-col ${compact ? "gap-1.5" : "gap-2"} sm:flex-row sm:items-start sm:justify-between`}>
              ...
              <div className={cn("flex shrink-0 items-center justify-between sm:flex-col sm:items-end", compact ? "gap-3" : "gap-4")}>
                <span className={cn("text-lg font-bold", meta.amountClassName)}>
...
export default function CashRegisterMovementsDialog({
  ...
  const canConfirm = hasSource && hasTarget && totalsMatch && !sourceAvailabilityExceeded && !registering;
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      await onRegister({
        ...
        reason: trimmedReason || "Cambio de denominación",
        detail: {
        ...
        <div className="space-y-4">
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            <div className="rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3">
              ...
              <p className="mt-1 text-2xl font-bold text-emerald-700 tabular-nums">${totalTo.toFixed(2)}</p>
            </div>
            <div className="rounded-2xl border border-sky-200 bg-sky-50 px-4 py-3">
              <p className="text-xs uppercase tracking-wide text-sky-700">Sale de caja</p>
              <p className="mt-1 text-2xl font-bold text-sky-700 tabular-nums">${totalFrom.toFixed(2)}</p>
            </div>
          ...
          {(hasSource || hasTarget) && !totalsMatch && (
            <div className="flex items-center gap-2 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">
              ...
              El total que ingresa a caja y el total que sale de caja deben ser exactamente iguales.
            </div>
          )}
