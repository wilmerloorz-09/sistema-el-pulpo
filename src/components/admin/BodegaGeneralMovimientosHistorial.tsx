...
type BodegaGeneralMovimientosHistorialProps = {
  filtroProducto?: string;
};

const BodegaGeneralMovimientosHistorial = ({
  filtroProducto = "",
}: BodegaGeneralMovimientosHistorialProps) => {
  const historialQuery = useQuery({
    queryKey: ["admin-movimientos-bodega-general"],
    queryFn: async (): Promise<MovimientoHistorialRow[]> => {
      const { data, error } = await supabase
        .from("movimientos_bodega_general" as any)
        .select(`
          id,
          tipo_movimiento,
          cantidad_movimiento,
      ...
      return ((data as any[]) ?? []).map((row) => ({
        ...
        productoNombre:
          (row.productos_globales as { nombre_principal: string } | null)?.nombre_principal
          ?? "Producto",
      }));
    },
  });
      ...
      {historialQuery.isLoading ? (
        ...
        <div className="divide-y divide-border/60">
          {rows.map((row) => (
            <div
              key={row.id}
              className="grid gap-2 px-4 py-3 sm:grid-cols-[minmax(0,1.2fr)_6rem_7rem_minmax(0,1fr)] sm:items-center"
            >
...
export default BodegaGeneralMovimientosHistorial;
