...
const SubcategoriesCrud = () => {
  ...
  const moveMutation = useMutation({
    mutationFn: async ({ current, target }: { current: Subcategory; target: Subcategory }) => {
      ...
      let error = (await supabase.from("subcategories").update({ display_order: tempOrder } as any).eq("id", current.id)).error;
      if (error) throw error;

      error = (await supabase.from("subcategories").update({ display_order: current.display_order } as any).eq("id", target.id)).error;
      if (error) throw error;

      error = (await supabase.from("subcategories").update({ display_order: target.display_order } as any).eq("id", current.id)).error;
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["admin-subcategories"] });
