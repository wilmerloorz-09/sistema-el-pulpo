-- lote-03-paid-at-blindaje
-- Ejecutar en Supabase SQL Editor (produccion)
-- Fecha: 2026-08-30

-- ===== 20260826120000_never_leave_paid_orders_without_paid_at.sql =====
-- =============================================================================
-- Blindaje: cobro completo NUNCA debe dejar la cabecera sin paid_at
-- (caso Pulpo 1 #123: payment OK + items paid_at, pero order.paid_at null â†’ bloqueÃ³ caja final)
-- =============================================================================
...
-- 2) Reparar Ã³rdenes del turno abierto con cobro completo pero paid_at null
...
BEGIN
  ...
  IF v_existing_count = v_payment_count THEN
    -- Pago ya existÃ­a: igual sincronizar cabecera (evita paid_at null)
    PERFORM public.sync_order_payment_state_internal(v_order_id);
    RETURN;
  END IF;
...
AS $function$
  ...
  -- Solo PAID no-especiales se evalÃºan por despacho pendiente
  paid_candidates AS (
    SELECT so.id
...
BEGIN
  ...
  -- Otras cajas "reales" que seguirÃ­an abiertas si cerramos esta
  SELECT COUNT(*)::int
  ...
  IF v_other_real_open = 0 THEN
    ...
    IF v_unpaid_count > 0 THEN
      ...
      RAISE EXCEPTION
        'No puedes cerrar la caja porque es la Ãºltima abierta y aÃºn hay Ã³rdenes por cobrar.%s%s',
        E'\n\nÃ“rdenes sin pagar: ' || COALESCE(v_unpaid_preview, ''),
        CASE
          WHEN v_unpaid_count > 20 THEN E'\nâ€¦ y ' || (v_unpaid_count - 20)::text || ' mÃ¡s'
          ELSE ''
        END;
    END IF;
  END IF;
