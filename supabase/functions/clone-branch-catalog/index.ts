...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { source_branch_id, target_branch_id } = await req.json();
    ...
    // 5. Clone categories → subcategories → products (preserving hierarchy)
    ...
    if (cats?.length) {
      for (const cat of cats) {
        ...
        // Clone subcategories for this category
        ...
        if (subs?.length) {
          for (const sub of subs) {
            ...
            // Clone products for this subcategory
