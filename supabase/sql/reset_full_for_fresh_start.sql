...
--   - incluye el modulo `turno` como acceso propio para abrir/cerrar/configurar turno
--   - el supervisor por defecto conserva `turno: MANAGE` y no debe recibir `admin_sucursal` por defecto
--
-- PRECONDICION:
...
-- - TAMBIEN PERMANECE INTACTA LA SEPARACION DE PERMISOS DE TURNO:
--   - `turno` es modulo propio para supervisor
--   - `can_manage_shift_admin(...)` permite administrar turno sin exponer Administracion
--   - `cash_shifts` debe poder verse desde Turno por usuarios con `can_manage_shift_admin(...)`
--   - la politica de anulacion directa cargada desde Turno debe usar permisos de Turno, no solo `admin_sucursal`
-- - TAMBIEN PERMANECE INTACTA LA REGLA UI DE CAMBIO DE CAJERO:
--   - si el turno esta abierto y cambia el usuario con `can_use_caja`, Guardar debe pedir confirmacion
--   - la contrasena solicitada es la del usuario logueado que realiza el cambio
...
-- - modulo `turno`, helper `can_manage_shift_admin(...)` y roles/permisos base intactos
-- - supervisor base sin `admin_sucursal` por defecto y con acceso a Turno
