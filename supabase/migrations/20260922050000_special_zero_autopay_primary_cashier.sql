-- Autopago de especial $0: atribuir el cobro al cajero principal del turno
-- (cash_shifts.primary_cashier_id), no a quien envió/despachó.
...
BEGIN
  ...
  -- Cajero principal del turno de la orden; si no hay, del turno OPEN de la sucursal.
  ...
  IF v_primary_cashier_id IS NULL THEN
    SELECT cs.primary_cashier_id
    INTO v_primary_cashier_id
    FROM public.cash_shifts cs
    WHERE cs.branch_id = v_order.branch_id
      AND cs.status = 'OPEN'
      AND cs.primary_cashier_id IS NOT NULL
    ORDER BY cs.opened_at DESC NULLS LAST, cs.id DESC
    LIMIT 1;
  END IF;

  -- Preferir cajero principal; fallback al actor / creador de la orden.
  v_actor_id := COALESCE(v_primary_cashier_id, v_actor_id, v_order.created_by);
  ...
  v_payments := jsonb_build_array(
    jsonb_build_object(
      ...
      'notes', format(
        ...
        v_group_id,
        CASE
          WHEN v_primary_cashier_id IS NOT NULL THEN '|ASSIGNED_PRIMARY_CASHIER:1'
          ELSE ''
        END
      ),
...
COMMENT ON FUNCTION public.autopagar_orden_especial_cero_interna(uuid, uuid, timestamptz) IS
  'Registra cobro $0 de especial. created_by = cajero principal del turno cuando existe.';
