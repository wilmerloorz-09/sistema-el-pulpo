-- Corrige ordenes re-etiquetadas al turno ABIERTO actual pero creadas/enviadas antes de su opened_at.
-- (Trigger antiguo en UPDATE de status; ver 20260509140000.)

UPDATE public.orders o
SET cash_shift_id = sub.correct_shift_id
FROM (
  SELECT
    o2.id AS order_id,
    (
      ...
      WHERE cs.branch_id = o2.branch_id
        AND cs.opened_at <= COALESCE(o2.sent_to_kitchen_at, o2.created_at)
        AND COALESCE(cs.closed_at, 'infinity'::timestamptz) >= COALESCE(o2.sent_to_kitchen_at, o2.created_at)
      ORDER BY cs.opened_at DESC
      LIMIT 1
    ) AS correct_shift_id
  FROM public.orders o2
  JOIN public.cash_shifts open_cs
    ON open_cs.id = o2.cash_shift_id
   AND open_cs.status = 'OPEN'
  WHERE COALESCE(o2.sent_to_kitchen_at, o2.created_at) < open_cs.opened_at
) sub
WHERE o.id = sub.order_id
  AND sub.correct_shift_id IS NOT NULL;

-- Parche minimo en overview de mesas: opened_at del turno abierto.
