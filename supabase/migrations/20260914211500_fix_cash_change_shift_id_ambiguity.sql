-- Corrige la ambiguedad entre la columna OUT shift_id y
-- cash_shift_denoms.shift_id al registrar cambios de denominacion.
...
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1
    FROM public.cash_register_openings AS cro
  ...
  RETURNING cash_register_movements.id INTO v_inserted_id;
  ...
  LEFT JOIN public.profiles AS recorder ON recorder.id = crm.recorded_by
  WHERE crm.id = v_inserted_id;
END;
$$;

GRANT EXECUTE ON FUNCTION public.registrar_movimiento_caja(uuid, text, numeric, text, jsonb)
TO authenticated;
