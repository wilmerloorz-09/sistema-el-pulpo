-- Agregar columna para plantilla de caja secundaria individual en cash_shift_users
ALTER TABLE public.cash_shift_users
  ADD COLUMN IF NOT EXISTS secondary_caja_template_id uuid REFERENCES public.cash_register_templates(id);

COMMENT ON COLUMN public.cash_shift_users.secondary_caja_template_id IS
  'Plantilla especifica asignada a este cajero secundario en el turno. Si es nula, usa la general del turno.';

-- Modificar apply_shift_caja_configuration para leer template_id de JSON y aplicar arqueo
  ...
  IF v_secondary_enabled AND COALESCE(array_length(v_cashier_ids, 1), 0) > 0 THEN
    ...
    LOOP
      IF NOT EXISTS (
        ...
        -- Get the specific template for this user if set, otherwise fallback to general
