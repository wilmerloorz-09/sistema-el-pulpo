ALTER TABLE public.payment_void_requests
  ADD COLUMN IF NOT EXISTS payment_item_selections jsonb NULL,
  ADD COLUMN IF NOT EXISTS refund_amount numeric(10,2) NULL,
  ADD COLUMN IF NOT EXISTS cash_refund_detail jsonb NULL,
  ADD COLUMN IF NOT EXISTS replacement_payment_id uuid NULL REFERENCES public.payments(id) ON DELETE SET NULL;

DROP FUNCTION IF EXISTS public.request_void_payment(uuid, uuid, text, text);
...
DROP FUNCTION IF EXISTS public.approve_and_void_payment(uuid, uuid, text, uuid, uuid, uuid, text);
...
BEGIN
  ...
  IF ABS(v_refund_cash_total - v_refund_amount) > 0.01 THEN
    RAISE EXCEPTION 'La devolucion en efectivo no coincide con el valor a anular';
  END IF;
  ...
  LOOP
    ...
    SET qty_current = qty_current - v_cash_row.qty
    WHERE shift_id = p_current_shift_id
      AND denomination_id = v_cash_row.denomination_id;
...
REVOKE ALL ON FUNCTION public.approve_and_void_payment(uuid, uuid, text, uuid, uuid, uuid, text, jsonb, jsonb) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.approve_and_void_payment(uuid, uuid, text, uuid, uuid, uuid, text, jsonb, jsonb) TO authenticated;
