-- Asocia cada orden al turno de caja donde se opera y acota listados al turno abierto.
-- Ademas, el cierre de turno debe bloquear ordenes PAID pendientes (despacho / mesa).

ALTER TABLE public.orders
  ADD COLUMN IF NOT EXISTS cash_shift_id uuid REFERENCES public.cash_shifts(id);

CREATE INDEX IF NOT EXISTS idx_orders_branch_cash_shift
  ON public.orders (branch_id, cash_shift_id);

COMMENT ON COLUMN public.orders.cash_shift_id IS
  'Turno de caja al que pertenece la orden operativa; acota Mesas/Ordenes/Cocina al turno abierto.';

-- 1) Desde el primer pago con shift conocido
UPDATE public.orders o
SET cash_shift_id = s.shift_id
FROM (
  SELECT DISTINCT ON (p.order_id) p.order_id, p.shift_id
  FROM public.payments p
  WHERE p.shift_id IS NOT NULL
  ORDER BY p.order_id, p.created_at ASC
) s
WHERE o.id = s.order_id
  AND o.cash_shift_id IS NULL;

-- 2) Ventana de turno que contiene el primer envio a caja o creacion
UPDATE public.orders o
SET cash_shift_id = (
  SELECT cs.id
  FROM public.cash_shifts cs
  WHERE cs.branch_id = o.branch_id
    AND cs.opened_at <= COALESCE(o.sent_to_kitchen_at, o.created_at)
    AND COALESCE(cs.closed_at, 'infinity'::timestamptz) >= COALESCE(o.sent_to_kitchen_at, o.created_at)
  ORDER BY cs.opened_at DESC
  LIMIT 1
)
WHERE o.cash_shift_id IS NULL;

-- 3) Ultimo recurso: turno mas reciente ya iniciado a la fecha de creacion
UPDATE public.orders o
SET cash_shift_id = (
  SELECT cs.id
  FROM public.cash_shifts cs
  WHERE cs.branch_id = o.branch_id
    AND cs.opened_at <= o.created_at
  ORDER BY cs.opened_at DESC
  LIMIT 1
)
WHERE o.cash_shift_id IS NULL;

CREATE OR REPLACE FUNCTION public.orders_default_cash_shift_id()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
...
CREATE TRIGGER trg_orders_default_cash_shift_id
  BEFORE INSERT OR UPDATE OF branch_id, status, cash_shift_id
  ON public.orders
  FOR EACH ROW
...
GRANT EXECUTE ON FUNCTION public.create_successor_order_after_payment_void() TO authenticated;
