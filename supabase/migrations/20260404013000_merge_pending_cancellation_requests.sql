...
DECLARE
  ...
  v_existing_ids uuid[];
  v_existing_item_map jsonb := '{}'::jsonb;
...
BEGIN
  ...
  SELECT array_agg(id), max(notes)
  INTO v_existing_ids, v_existing_notes
  ...
  IF COALESCE(array_length(v_existing_ids, 1), 0) > 0 THEN
    SELECT COALESCE(
      jsonb_object_agg(order_item_id::text, total_qty),
      '{}'::jsonb
    )
    INTO v_existing_item_map
    FROM (
      SELECT
        oic.order_item_id,
        SUM(oic.quantity_cancelled)::int AS total_qty
      FROM public.order_item_cancellations oic
      WHERE oic.order_cancellation_id = ANY(v_existing_ids)
      GROUP BY oic.order_item_id
    ) aggregated;

    IF v_existing_item_map = '{}'::jsonb AND v_existing_notes IS NOT NULL THEN
      SELECT COALESCE(
        jsonb_object_agg(item.order_item_id, item.quantity_cancelled),
        '{}'::jsonb
      )
      INTO v_existing_item_map
      FROM (
        SELECT
          ...
          GREATEST(0, COALESCE((item->>'quantity_cancelled')::int, 0)) AS quantity_cancelled
        FROM jsonb_array_elements(
          COALESCE(
            (
              regexp_replace(v_existing_notes, '^\[PENDING_REQUEST\]\s*', '')
            )::jsonb -> 'items',
            '[]'::jsonb
          )
        ) item
      ...
      WHERE item.order_item_id IS NOT NULL
        AND item.order_item_id <> ''
        AND item.quantity_cancelled > 0;
    END IF;
  END IF;
  ...
  IF p_items IS NOT NULL AND jsonb_typeof(p_items) = 'array' THEN
    ...
    LOOP
      IF COALESCE(v_item->>'order_item_id', '') = '' THEN
        CONTINUE;
      END IF;

      v_existing_item_map := jsonb_set(
        v_existing_item_map,
        ARRAY[v_item->>'order_item_id'],
        to_jsonb(
          COALESCE((v_existing_item_map ->> (v_item->>'order_item_id'))::int, 0)
          + GREATEST(0, COALESCE((v_item->>'quantity_cancelled')::int, 0))
        ),
        true
      );
    END LOOP;
    ...
    'items', COALESCE(
      (
        SELECT jsonb_agg(
          jsonb_build_object(
            'order_item_id', key,
            'quantity_cancelled', value::int
          )
          ORDER BY key
        )
        FROM jsonb_each_text(v_existing_item_map)
        WHERE value::int > 0
      ),
      '[]'::jsonb
    )
  ...
  FOR v_item IN
    SELECT *
    FROM jsonb_array_elements(COALESCE(v_request_payload->'items', '[]'::jsonb))
  LOOP
