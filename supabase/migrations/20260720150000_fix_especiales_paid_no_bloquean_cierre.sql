-- =============================================================================
-- Fix: órdenes especiales PAID no deben bloquear el cierre de turno
-- =============================================================================
-- Síntoma: "Referencias: Orden especial, Orden especial" aunque la UI no
-- muestre pendientes. Causa: list_branch_closure_blocking_orders bloqueaba
-- TODA orden is_special con status = 'PAID' (aunque ya estuviera cobrada).
-- Eso contradice el flujo de especiales $0 (se marcan PAID y luego se cierra).
--
-- Regla corregida:
--   - Especiales PAID: no bloquean; al cerrar se finalizan a KITCHEN_DISPATCHED.
--   - Especiales SENT_TO_KITCHEN / READY / KITCHEN_DISPATCHED sin paid_at: sí.
--   - PAID no especiales: solo si tienen despacho pendiente (set-based).
