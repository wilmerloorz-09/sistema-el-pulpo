-- Actualizar RPC para permitir crear ordenes adicionales desde una orden PAGADA.
-- Esto es necesario en el flujo CASH_THEN_DISPATCH donde la mesa sigue ocupada esperando despacho.
...
BEGIN
  ...
  -- Cambiado: permitir PAID, bloquear solo CANCELLED o KITCHEN_DISPATCHED (si existiera como estado terminal de mesa)
  -- En este sistema, una orden de mesa termina su ciclo cuando se libera la mesa (status cambia de ocupada a libre).
  -- Permitimos crear ordenes hermanas incluso si la actual ya esta pagada.
  IF v_source_order.status IN ('CANCELLED', 'KITCHEN_DISPATCHED') THEN
