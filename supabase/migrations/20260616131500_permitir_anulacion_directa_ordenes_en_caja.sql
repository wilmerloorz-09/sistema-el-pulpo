-- Migration to allow direct item cancellation for SENT_TO_KITCHEN and DRAFT orders.
...
BEGIN
  ...
  -- Bypasseamos la autorización si la orden está en estado 'SENT_TO_KITCHEN' o 'DRAFT'
  IF v_order.status NOT IN ('SENT_TO_KITCHEN', 'DRAFT') AND NOT public.can_manage_branch_admin(v_actor_id, v_order.branch_id) THEN
    SELECT *
    INTO v_gate
