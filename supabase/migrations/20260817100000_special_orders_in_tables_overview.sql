-- Las órdenes especiales guardan la mesa en special_origin_table_id (table_id = NULL).
-- get_branch_tables_overview solo unía por table_id, así la mesa aparecía libre.
...
AS $$
  ...
  table_orders AS (
    SELECT
      o.id,
      COALESCE(o.table_id, o.special_origin_table_id) AS table_id,
    ...
    JOIN visible_tables vt
      ON vt.id = COALESCE(o.table_id, o.special_origin_table_id)
    ...
    WHERE o.branch_id = p_branch_id
      ...
      AND (
        (COALESCE(o.is_special, false) = false AND o.table_id IS NOT NULL)
        OR (COALESCE(o.is_special, false) = true AND o.special_origin_table_id IS NOT NULL)
      )
      AND (
    ...
    GROUP BY
      o.id,
      COALESCE(o.table_id, o.special_origin_table_id),
      o.table_order_position,
      o.status,
      o.created_at,
      o.updated_at,
  ...
  empty_drafts AS (
    ...
    WHERE to1.status = 'DRAFT'
      ...
      AND COALESCE(to1.is_special, false) = false
    ORDER BY
      to1.table_id,
  ...
  special_order_payments AS (
    ...
    JOIN public.payments p
      ON p.order_id = vo.id
    WHERE COALESCE(vo.is_special, false)
      AND vo.special_total_manual IS NOT NULL
  ...
  order_totals AS (
    SELECT
      ...
      vo.total_items,
      CASE
        WHEN COALESCE(vo.is_special, false) AND vo.special_total_manual IS NOT NULL THEN
          GREATEST(
            0,
            ROUND(vo.special_total_manual::numeric - COALESCE(sop.paid_total, 0), 2)
          )
        ELSE
          ROUND(
            COALESCE(
    ...
    LEFT JOIN special_order_payments sop
      ON sop.order_id = vo.id
    GROUP BY
      vo.id,
      vo.table_id,
      vo.table_order_position,
  ...
  SELECT
    ...
    COALESCE((
      SELECT vo.is_special
      FROM visible_orders vo
      WHERE vo.id = COALESCE(ro.order_id, ed.draft_order_id)
      LIMIT 1
    ), false) AS is_special
