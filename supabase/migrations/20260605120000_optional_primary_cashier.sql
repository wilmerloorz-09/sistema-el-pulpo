-- Cajero principal opcional; al menos un cajero (principal o secundario) en la configuración de caja.

DROP FUNCTION IF EXISTS public.apply_shift_caja_configuration(uuid, uuid, uuid, boolean, uuid, uuid[], jsonb);
DROP FUNCTION IF EXISTS public.open_cash_shift_with_tables(uuid, uuid, integer, public.shift_user_input[], uuid, boolean, uuid, uuid[], jsonb);
...
DECLARE
  ...
  v_cashier_count integer := 0;
  v_secondary_ids uuid[] := COALESCE(p_secondary_cashier_ids, ARRAY[]::uuid[]);
BEGIN
  ...
  IF COALESCE(p_secondary_cajas_enabled, false) THEN
    v_cashier_count := v_cashier_count + COALESCE(array_length(v_secondary_ids, 1), 0);
  END IF;
  ...
  PERFORM public.apply_shift_caja_configuration(
    ...
    p_secondary_cashier_ids := v_secondary_ids,
