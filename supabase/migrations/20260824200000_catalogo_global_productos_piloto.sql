-- Catálogo global de productos (piloto aislado a sucursales con usa_catalogo_global = true).
...
COMMENT ON TABLE public.productos_globales IS
  'Catálogo único del sistema. Defaults para menú; el precio/nombre finales viven en menu_nodes.';
...
-- 3) Producto × sucursal (+ inventario operativo)
...
COMMENT ON TABLE public.producto_sucursal IS
  'Asignación del producto global a una sucursal + stock e integra_con_ventas.';
...
COMMENT ON COLUMN public.menu_nodes.producto_global_id IS
  'FK al catálogo global (piloto). En sucursales legacy permanece NULL.';

-- 5) Subcategoría puente para fila en products (mismo id = producto global)
...
BEGIN
  ...
  WHERE c.branch_id = v_branch
    AND c.description = 'Sistema · Catálogo global'
    AND s.description = 'Catálogo global'
  LIMIT 1;
  RETURN v_sub;
END;
$$;

-- 6) Al crear/actualizar producto global → espejo en products (mismo UUID) para POS/inventario legacy
...
-- 7) producto_sucursal ↔ inventario_productos (para que submit/Ordenes no cambien)
...
-- Si movimientos/admin tocan inventario_productos, reflejar en producto_sucursal (solo si existe asignación)
...
-- 9) Exponer flag en access context (sin cambiar lógica de otras sucursales)
