...
BEGIN
  ...
  -- 1. Check if CURRENT payment is already voided
  IF COALESCE(lower(v_payment.status), 'active') = 'voided'
    OR v_payment.voided_at IS NOT NULL
    OR COALESCE(v_payment.notes, '') ILIKE '%VOID_REQUESTED:%'
    OR COALESCE(v_payment.notes, '') ILIKE '%VOIDED:%'
  THEN
    error_code := 'PAYMENT_ALREADY_VOIDED';
    error_message := 'El pago ya fue anulado o tiene una solicitud activa';
    RETURN NEXT;
    RETURN;
  END IF;

  -- 2. NEW CRITICAL CHECK: Single void per order
  -- Check if ANY other payment for this order is already voided
  SELECT EXISTS (
  ...
  ) INTO v_other_voided_exists;

  IF v_other_voided_exists THEN
    error_code := 'ORDER_ALREADY_HAS_VOIDED_PAYMENT';
    error_message := 'Esta cuenta ya tiene un pago anulado. Por politica de seguridad, solo se permite una anulacion por orden.';
    RETURN NEXT;
    RETURN;
  END IF;
