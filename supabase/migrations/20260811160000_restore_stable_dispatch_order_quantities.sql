-- =============================================================================
-- HOTFIX: restaurar dispatch_order_quantities estable
-- =============================================================================
-- La migración 20260811140000 reescri el despacho (temp tables + compact diferido
-- + skip sync si PAID). Bajo carga / pooler eso provocaba fallo o rollback del
-- RPC: la UI quitaba el ítem (optimista) y al fallar volvía a aparecer.
--
-- Se restaura la lógica probada de 20260709120000 (especial $0 incluida).
-- Compact diferido / snapshot único en sync (150000) se mantienen; el despacho
-- vuelve a recompute + sync directos como antes.
-- =============================================================================
...
-- Desactivar cola diferida: siempre compactar en el momento (evita fallos raros
-- con temp tables / pooler que revertían todo el despacho).
