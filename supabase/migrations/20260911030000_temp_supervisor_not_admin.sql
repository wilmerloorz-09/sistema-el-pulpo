-- Supervisor temporal = supervisor permanente, NO administrador.
-- Quita admin_sucursal / can_manage_branch_admin por delegacion.
-- Conserva turno MANAGE + modulos operativos del rol supervisor + entrada sin habilitado.
...
BEGIN
  ...
  -- Sucursal donde hoy es supervisor temporal.
  SELECT d.branch_id
  ...
  -- Si hay delegacion activa y la sucursal actual no es esa, priorizar la delegada.
  ...
  IF v_active_branch IS NOT NULL THEN
    ...
    IF NOT v_is_global_admin THEN
      ...
      -- Igual al rol supervisor (NO admin_sucursal).
      IF v_is_delegated_supervisor THEN
        v_permissions := jsonb_build_object(
          ...
          'caja', 'VIEW',
          'jornadas_personal', 'OPERATE',
          'reportes_sucursal', 'VIEW'
