-- Express: mismo menu que para llevar, flujo despacho -> cobro (sin cobro parcial).

DO $$
BEGIN
  ALTER TYPE public.order_type ADD VALUE 'EXPRESS';
EXCEPTION
...
SET express_enabled = COALESCE(express_enabled, true)
WHERE express_enabled IS NULL;

CREATE OR REPLACE FUNCTION public.create_express_order(
  p_branch_id uuid,
  p_created_by uuid
)
RETURNS uuid
...
BEGIN
  ...
  IF (
    ...
    RAISE EXCEPTION 'No tienes permisos para abrir ordenes Express en este turno.';
  END IF;
...
REVOKE ALL ON FUNCTION public.create_express_order(uuid, uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.create_express_order(uuid, uuid) TO authenticated;
...
REVOKE ALL ON FUNCTION public.submit_express_order_draft_items(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.submit_express_order_draft_items(uuid) TO authenticated;

-- Despacho Express: permitir despacho sin cobro previo.
...
BEGIN
  ...
  IF v_is_express THEN
    IF v_order.status NOT IN ('SENT_TO_KITCHEN', 'READY', 'KITCHEN_DISPATCHED') THEN
  ...
  IF p_operation_type = 'total' THEN
    ...
    SELECT
      ...
      CASE
        WHEN v_is_express THEN snapshot.quantity_dispatched_available
        ELSE LEAST(
