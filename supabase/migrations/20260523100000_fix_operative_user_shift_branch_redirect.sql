-- Fix: Usuarios operativos con sucursal por defecto distinta a la del turno
-- El problema: usuario1 tiene "Local Principal" como active_branch_id, pero está
-- habilitado en un turno de OTRA sucursal. La función nunca cambia porque la
-- sucursal actual es válida (tiene rol allí). Pero no tiene turno allí.
--
-- La solución: Si el usuario tiene un turno abierto en otra sucursal y NO tiene
-- turno en la sucursal actual, forzar el cambio a la sucursal del turno.
...
BEGIN
  ...
  -- 1. Sucursal activa guardada en perfil
  SELECT active_branch_id INTO v_active_branch
  FROM public.profiles
  WHERE id = v_user_id;

  -- 2. Buscar si el usuario tiene un turno abierto donde está habilitado
  ...
  -- 3. Verificar si tiene turno en la sucursal actual
  ...
  -- 4. REDIRECCION: Si tiene turno en otra sucursal pero NO en la actual, cambiar
  --    SOLO si el usuario NO es admin (los admins pueden estar sin turno)
  IF v_shift_branch IS NOT NULL
     AND NOT v_has_shift_at_current
     AND NOT public.is_global_admin(v_user_id)
  THEN
  ...
  -- 5. Sucursales accesibles (roles fijos + turnos abiertos)
  ...
  -- 6. Si no hay sucursal activa o la actual no es accesible, auto-seleccionar
  ...
  IF v_active_branch IS NOT NULL THEN
    ...
    -- Permisos por turno (inyección dinámica)
    ...
    -- Shift permissions tienen prioridad sobre roles fijos para operativos
