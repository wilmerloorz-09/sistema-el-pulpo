-- Add new RPC to request an order cancellation
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE ub.branch_id = v_order.branch_id
      AND ub.user_id = p_user_id
      AND p.is_active = true
  ) THEN
    RAISE EXCEPTION 'El usuario no tiene acceso a la sucursal de esta orden';
  END IF;
...
-- Ensure cancel_order_quantities clears the cancellation request
...
BEGIN
  ...
  -- 3) Loop each target again to insert order_item_cancellations based on stage availability
  FOR v_target_order_item_id, v_target_qty IN
    SELECT order_item_id, quantity_cancelled FROM tmp_cancel_targets
  LOOP
  ...
  -- 6) Recompute operational state
  PERFORM public.recompute_order_operational_state(p_order_id);

  -- 7) If order goes CANCELLED completely, mark it so and CLEAR ANY PENDING CANCELLATION REQUEST
  UPDATE public.orders
  SET
