-- =============================================================================
-- Corrección de pre-pagos en Dine In (Mesa) antes de despacho de cocina
-- =============================================================================
--
-- Problema: La migración 20260624220000 introdujo una regla estricta donde un ítem
-- de mesa solo se considera pagado si ha sido despachado (payable_qty > 0). Esto
-- evitaba la regresión de marcar órdenes vacías como PAID, pero impedía que las
-- órdenes pagadas por completo antes de ser despachadas (ej. cobradas al inicio)
-- cambiaran de estado a PAID. Como consecuencia, paid_at permanecía NULL,
-- impidiendo la generación del token de promoción y su impresión QR.
--
-- Solución: Habilitar que un ítem se considere pagado si el total cobrado por
-- pagos (quantity_paid_from_payments) cubre la cantidad ordenada del ítem,
-- independientemente de si ya fue despachado o no.
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(
      BOOL_AND(
        ...
        OR item_state.quantity_paid_from_payments >= GREATEST(0, item_state.quantity_ordered - item_state.quantity_cancelled_total)
        OR (
  ...
  IF COALESCE(v_order.is_special, false) IS NOT TRUE THEN
    ...
    SET paid_at = CASE
      WHEN GREATEST(0, item_state.quantity_ordered - item_state.quantity_cancelled_total) <= 0
        OR item_state.quantity_paid_from_payments >= GREATEST(0, item_state.quantity_ordered - item_state.quantity_cancelled_total)
        OR (
          item_state.payable_qty > 0
          AND item_state.paid_qty_effective >= item_state.payable_qty
...
-- Backfill: Sincronizar órdenes Dine In cobradas en su totalidad que se quedaron atascadas en SENT_TO_KITCHEN o READY
DO $$
DECLARE
  r record;
  v_count integer := 0;
BEGIN
  FOR r IN
    ...
    WHERE o.status IN ('SENT_TO_KITCHEN', 'READY')
      ...
      -- Validar que el total pagado cubra el total calculado de la orden
      AND public.order_active_payments_total(o.id) >= (
        SELECT COALESCE(SUM(
          GREATEST(0, oi.quantity - COALESCE(snapshot.quantity_cancelled_total, 0)) * oi.unit_price
        ), 0)
        FROM public.order_items oi
        LEFT JOIN public.get_order_operational_snapshot(o.id) snapshot
          ON snapshot.order_item_id = oi.id
        WHERE oi.order_id = o.id
      )
      -- Asegurar que haya pagos registrados activos
  ...
  RAISE NOTICE 'Ordenes pre-pagadas sincronizadas: %', v_count;
END;
$$;
