-- =============================================
-- Defer order_number and order_code assignment
-- to kitchen submission (submit_order_draft_items)
-- instead of assigning at order creation (DRAFT).
-- =============================================

-- 1) Make order_number nullable and remove auto-increment default
--    The sequence orders_order_number_seq still exists for manual use.
...
-- 2) Set existing DRAFT orders (that have no items) to NULL number/code
--    since they were never confirmed. This cleans up ghost drafts.
UPDATE public.orders
SET order_number = NULL, order_code = NULL
WHERE status = 'DRAFT'
  AND NOT EXISTS (
    SELECT 1 FROM public.order_items oi WHERE oi.order_id = orders.id
  );

-- 3) Update generate_order_code trigger to SKIP drafts
...
BEGIN
  ...
  -- Skip if this is a draft order — number will be assigned on kitchen submission
...
-- 4) Update submit_order_draft_items to assign order_number + order_code
--    when the order transitions out of DRAFT for the first time
...
BEGIN
  ...
  -- Assign order_number and order_code if not yet assigned (first submission)
