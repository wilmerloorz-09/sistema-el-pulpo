-- Supervisor temporal: mismos privilegios de entrada que supervisor/admin de sucursal.
-- Si hay delegacion activa del dia, el gate lo trata como habilitado aunque no figure
-- (o este deshabilitado) en cash_shift_users, y sincroniza el turno abierto.
...
BEGIN
  ...
  -- Autocurar: si es supervisor temporal y hay turno abierto, dejarlo habilitado en el turno.
  IF v_is_delegated_supervisor THEN
    PERFORM public.apply_supervisor_delegation_to_open_shift(p_branch_id, auth.uid());
  END IF;
  ...
  -- Admin de sucursal O supervisor temporal: entrada operativa completa.
