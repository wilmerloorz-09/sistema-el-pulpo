-- =============================================================================
-- Fix: token_promocion debe persistir tras despacho (KITCHEN_DISPATCHED)
-- =============================================================================
-- Problema: orden_promocion_token_trigger borraba token_promocion cuando la orden
-- dejaba status = 'PAID' (p. ej. al pasar a KITCHEN_DISPATCHED tras despacho).
-- El QR impreso en el recibo quedaba inválido minutos después del cobro.
--
-- Solución:
--   1. El token se genera al quedar paid_at y se conserva mientras haya pago.
--   2. Solo se anula si paid_at pasa a NULL o la orden se cancela.
--   3. validar_token_promocion_cliente acepta órdenes pagadas en PAID o
--      KITCHEN_DISPATCHED.
--   4. Backfill de órdenes con paid_at pero sin token.
-- =============================================================================
...
AS $$
BEGIN
  -- Orden pagada: generar token una sola vez
  IF NEW.paid_at IS NOT NULL AND NEW.status <> 'CANCELLED' THEN
    ...
    END IF;
  -- Anulación de pago o cancelación: limpiar token
  ELSIF NEW.paid_at IS NULL OR NEW.status = 'CANCELLED' THEN
    NEW.token_promocion := NULL;
  END IF;
  -- PAID -> KITCHEN_DISPATCHED u otros estados post-pago: conservar token existente
  RETURN NEW;
END;
$$;
...
BEGIN
  ...
  -- Aceptar órdenes pagadas aunque ya estén despachadas
  IF v_orden.paid_at IS NULL OR v_orden.status = 'CANCELLED' THEN
  ...
  WHERE order_id = v_orden.id
    AND COALESCE(notes, '') NOT ILIKE '%REVERSED:%'
    AND COALESCE(notes, '') NOT ILIKE '%VOIDED:%'
    AND COALESCE(notes, '') NOT ILIKE '%TRANSFER_PROOF_PENDING:1%';
...
-- Backfill: órdenes pagadas sin token (p. ej. token borrado al despachar)
DO $$
DECLARE
  v_orden record;
BEGIN
  FOR v_orden IN
    ...
    WHERE paid_at IS NOT NULL
      AND token_promocion IS NULL
      AND status <> 'CANCELLED'
  LOOP
    UPDATE public.orders
    SET token_promocion = public.generar_token_promocion_unico()
    WHERE id = v_orden.id;
  END LOOP;
END;
$$;
