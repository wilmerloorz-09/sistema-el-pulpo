ALTER TABLE public.cash_register_movements
  ADD COLUMN IF NOT EXISTS movement_detail jsonb NULL;

DROP FUNCTION IF EXISTS public.list_cash_register_movements(uuid);
...
BEGIN
  ...
  IF v_tipo = 'cambio_denominacion' THEN
    ...
    IF v_from_total <= 0 OR v_to_total <= 0 THEN
      RAISE EXCEPTION 'El detalle del cambio debe incluir denominaciones de origen y destino';
    END IF;
