...
DECLARE
  ...
  v_gate record;
  v_can_direct_by_turn boolean := false;
  v_requires_authorization boolean := false;
  v_is_supervisor_by_turn boolean := false;
BEGIN
  ...
  IF NOT public.can_manage_branch_admin(v_actor_id, v_order.branch_id) THEN
    ...
    v_is_supervisor_by_turn := COALESCE(v_gate.is_supervisor, false);
    v_can_direct_by_turn := COALESCE(v_gate.can_authorize_order_cancel, false) OR v_is_supervisor_by_turn;

    IF NOT v_is_supervisor_by_turn THEN
      SELECT EXISTS (
        SELECT 1
    ...
    IF NOT v_can_direct_by_turn OR v_requires_authorization THEN
