-- Extra: menu de mesas (TABLE) sin categoria PLATOS, sin mesa, flujo caja -> pago -> despacho.

DO $$
BEGIN
  ALTER TYPE public.order_type ADD VALUE 'EXTRA';
EXCEPTION
...
BEGIN
  ...
  IF (
    ...
    RAISE EXCEPTION 'No tienes permisos para abrir ordenes Extra en este turno.';
  END IF;
...
REVOKE ALL ON FUNCTION public.create_extra_order(uuid, uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.create_extra_order(uuid, uuid) TO authenticated;
