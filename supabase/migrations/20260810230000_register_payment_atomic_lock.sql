-- =============================================================================
-- Cobro atómico: lock de orden/ítems + tope de cantidades + idempotencia
-- =============================================================================
-- Evita sobrecupros cuando dos cajeros (o un reintento de red) registran el
-- mismo cobro en paralelo. Mantiene la firma register_payment_with_items(jsonb,jsonb).
-- =============================================================================
...
BEGIN
  ...
  -- Bloquear cabecera de orden para serializar cobros concurrentes
  PERFORM 1
  ...
  -- Reintento idempotente: todos los payment.id ya existen
  IF v_existing_count = v_payment_count THEN
    RETURN;
  END IF;

  -- Estado intermedio / parcial: no mezclar ids nuevos con ya insertados
  IF v_existing_count > 0 THEN
    RAISE EXCEPTION 'Pago parcial ya registrado; reintenta el cobro completo';
  END IF;

  -- Validar que no se cobren más unidades de las pendientes
  FOR v_item IN
    SELECT
