...
BEGIN
  FOR v_constraint IN
    SELECT conname
    FROM pg_constraint
    WHERE conrelid = 'public.order_item_dispatch_events'::regclass
      AND contype = 'c'
      AND pg_get_constraintdef(oid) ILIKE '%source_stage%'
  LOOP
    EXECUTE format('ALTER TABLE public.order_item_dispatch_events DROP CONSTRAINT %I', v_constraint.conname);
  END LOOP;
END
$$;

ALTER TABLE public.order_item_dispatch_events
  ALTER COLUMN source_stage SET NOT NULL;

ALTER TABLE public.order_item_dispatch_events
  ADD CONSTRAINT order_item_dispatch_events_source_stage_check
...
DROP FUNCTION IF EXISTS public.mark_order_quantities_ready(uuid, uuid, jsonb, text, text, text);
DROP FUNCTION IF EXISTS public.dispatch_order_quantities(uuid, uuid, jsonb, text, text, text);
