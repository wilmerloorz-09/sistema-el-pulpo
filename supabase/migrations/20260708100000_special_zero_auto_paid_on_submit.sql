-- Orden especial con total manual explícito en $0: al enviar borradores queda PAID
-- sin pasar por caja (no aplica si special_total_manual es NULL).
...
BEGIN
  ...
  v_next_status := CASE
    WHEN v_especial_cero THEN 'PAID'::public.order_status
  ...
  SET
    ...
    paid_at = CASE WHEN v_especial_cero THEN v_now ELSE NULL END,
    dispatched_at = NULL,
    updated_at = v_now
