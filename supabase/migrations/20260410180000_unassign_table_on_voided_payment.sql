-- Migration to automatically unassign tables from orders when a payment is voided
-- This fulfills the requirement: "ya no en mesa a la cual corresponde"
...
BEGIN
  ...
  -- We unassign the table from the order so the table becomes free
  -- and the order is moved to the "Voided Payments" section.
  UPDATE public.orders
  SET table_name_snapshot = COALESCE(v_order.table_name_snapshot, (SELECT name FROM public.restaurant_tables WHERE id = table_id)),
      table_id = NULL,
      updated_at = now()
  WHERE id = v_order.id;
