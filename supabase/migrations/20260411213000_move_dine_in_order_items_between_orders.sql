...
BEGIN
  ...
  LOOP
    ...
    IF COALESCE(v_snapshot.quantity_paid, 0) > 0 OR COALESCE(v_snapshot.quantity_cancelled_total, 0) > 0 THEN
      RAISE EXCEPTION 'El item % tiene pagos o anulaciones parciales y no se puede mover', v_source_item_id;
    END IF;

    IF v_requested_qty > (
      COALESCE(v_snapshot.quantity_pending_prepare, 0)
      + COALESCE(v_snapshot.quantity_ready_available, 0)
      + COALESCE(v_snapshot.quantity_dispatched_available, 0)
    ) THEN
      RAISE EXCEPTION 'No puedes mover mas cantidad de la disponible para item %', v_source_item_id;
    END IF;
    ...
    VALUES (
      ...
      CASE
        ...
        WHEN v_move_ready_history > 0 THEN 'READY'
