-- Revierte una regresion introducida con migraciones locales que reemplazaban
-- get_my_access_context por una version que vaciaba sucursales/permissos cuando
-- no habia turno (rompiendo entrada y menu para todos los operativos).
-- Estado restaurado: misma funcion que en 20260501100000_apply_branch_workflow_to_order_submission.sql (solo la RPC).
...
-- RPC auxiliar de migraciones experimentales; ya no se usa en el cliente.
DROP FUNCTION IF EXISTS public.get_my_open_shift_branch_id();
