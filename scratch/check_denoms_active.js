...
async function checkDenomsActive() {
  const { data: denoms } = await supabase.from('denominations').select('id, label, is_active');
  console.log('Denominations Active Status:', JSON.stringify(denoms, null, 2));
}
