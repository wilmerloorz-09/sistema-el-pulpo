...
- **Refresco operativo entre módulos (2026-08-02 → 2026-08-11):** hub `branch-ops-hub` invalida `OPERATIONAL_ORDER_LIST_KEYS` completo; poll backup **45 s** si Realtime cae; safety **30 s** solo Despacho/Servir; badge Sync lenta. Cola: RPC bundle + fetch directo (sin cache RQ). Detalle: `docs/operational-module-refresh.md`.
- **Resiliencia bajo saturación (2026-08-11):** Auth no borra `profile` ante timeout ni refetch en `TOKEN_REFRESHED`; BranchContext no sobrescribe acceso usable con payload vacío.
